...
class StandardFiltersTest < Minitest::Test
  ...
  def test_map
    ...
    assert_template_result 'abc', "{{ ary | map:'foo' | map:'bar' | join: '' }}",
      'ary' => [{ 'foo' => { 'bar' => 'a' } }, { 'foo' => { 'bar' => 'b' } }, { 'foo' => { 'bar' => 'c' } }]
  end

  def test_map_doesnt_call_arbitrary_stuff
    assert_template_result "[null]", '{{ "foo" | map: "__id__" }}'
    assert_template_result "", '{{ "foo" | map: "__id__" | join }}'
    assert_template_result "[null]", '{{ "foo" | map: "inspect" }}'
    assert_template_result "", '{{ "foo" | map: "inspect" | join }}'
  end

  def test_map_calls_to_liquid
    t = TestThing.new
    assert_template_result "[\"woot: 1\"]", '{{ foo | map: "whatever" }}', "foo" => [t]
    assert_template_result "woot: 2", '{{ foo | map: "whatever" | join }}', "foo" => [t]
  end

  def test_map_on_hashes
    assert_template_result "[42,17]", '{{ thing | map: "foo" | map: "bar" }}',
      "thing" => { "foo" => [ { "bar" => 42 }, { "bar" => 17 } ] }
    assert_template_result "4217", '{{ thing | map: "foo" | map: "bar" | join: "" }}',
      "thing" => { "foo" => [ { "bar" => 42 }, { "bar" => 17 } ] }
  end

  def test_legacy_map_on_hashes_with_dynamic_key
    hash = { "foo" => { "bar" => 42 } }
    assert_template_result "[42]", "{% assign key = 'foo' %}{{ thing | map: key | map: 'bar' }}", "thing" => hash
    assert_template_result "42", "{% assign key = 'foo' %}{{ thing | map: key | map: 'bar' | join }}", "thing" => hash
  end
  ...
  def test_map_over_proc
    drop = TestDrop.new
    p = proc{ drop }
    assert_template_result "[\"testfoo\"]", '{{ procs | map: "test" }}', "procs" => [p]
    assert_template_result "testfoo", '{{ procs | map: "test" | join: "" }}', "procs" => [p]
  end
  ...
  def test_map_over_drops_returning_procs
    ...
    assert_template_result "[\"foo\",\"bar\"]", '{{ drops | map: "proc" }}', "drops" => drops
    assert_template_result "foobar", '{{ drops | map: "proc" | join: "" }}', "drops" => drops
  end

  def test_map_works_on_enumerables
    assert_template_result "[1,2,3]", '{{ foo | map: "foo" }}', "foo" => TestEnumerable.new
    assert_template_result "123", '{{ foo | map: "foo" | join: "" }}', "foo" => TestEnumerable.new
  end

  def test_sort_works_on_enumerables
    assert_template_result "[2,1,3]", '{{ foo | sort: "bar" | map: "foo" }}', "foo" => TestEnumerable.new
    assert_template_result "213", '{{ foo | sort: "bar" | map: "foo" | join: "" }}', "foo" => TestEnumerable.new
  end
